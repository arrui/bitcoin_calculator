...
# Version code:2.0
# 命令使用方法 btc.sh [10000算力btc每天收益] [当前一个btc价格] [1算力成本（默认838）] [电费成本（单位kw/h，默认0.35）] [美元汇率（默认6.5）]
if [ $# -lt 2 ] ; then
echo "命令使用方法 btc.sh [10000算力btc每天收益] [当前一个btc价格] [1算力成本（默认838）] [电费成本（单位kw/h，默认0.35）] [美元汇率（默认6.5）]"
exit 0
fi
# 参数1 =======>> 10000算力btc每天收益（单位个）
# 参数2 =======>> 当前一个btc价格
# 参数3 =======>> 1算力成本
# 参数4 =======>> 电费成本（单位kw/h，默认0.35）
# 参数5 =======>> 美元汇率（默认6.5）
# 美元汇率（默认6.5）
if [ ! -n '$5' ];then
	USD_rate=$5
...
if [ ! -n '$1' ];then
	...
	btc_l_price=$(echo "$total_fee_one_day / $1" |bc)
	echo "btc保底价格:" $btc_l_price "RMB" "≈" $(echo "$btc_l_price/$USD_rate"|bc)"USD"
	echo "btc实时价格:" $2 "RMB" "≈" $(echo "$2/$USD_rate"|bc)"USD"
	echo "btc当前溢价:" $(echo "$(echo "scale=2; $2 / $btc_l_price"|bc)*100"|bc) "%"
	
	profit_one_day=$(echo "$(echo "$2 * $1"|bc) - $total_fee_one_day"|bc)
